...
def check_user_is_member(function):
    def wrap(request, *args, **kwargs):
        ...
        if blog.paid == True:
            if request.user.is_kemet_vip == True or blog.user == request.user:
...
def check_user_status(function):
    def wrap(request, *args, **kwargs):
        if request.user.is_kemet_vip == True:
            today= datetime.date.today()
            payment=Blog_Payment.objects.filter(user=request.user,type=2,expired=False,status="approved").last()
            if payment.expired_at <= today:
                payment.expired=True
                payment.save()
                request.user.is_kemet_vip =False
                request.user.save()
            ...
            if Blog_Payment.objects.filter(user=request.user,type=2,expired=False).exclude(Q(status="refund") | Q(status="declined")).select_related("user").exists():
...
def check_blogs_payment_status(function):
    def wrap(request, *args, **kwargs):
        ...
        if request.user.is_kemet_vip == True:
            today= datetime.date.today()
            payment=Blog_Payment.objects.filter(user=request.user,type=2,expired=False,status="approved").select_related("user").last()
            if payment.expired_at <= today:
                payment.expired=True
                payment.save()
                request.user.is_kemet_vip =False
