...
def audio(request):
    tracks=Audio_Book_Tracks.objects.filter(status="approved").prefetch_related("book")
    paginator = Paginator(tracks, 9) 
    page_number = request.GET.get('page')
    page_obj = paginator.get_page(page_number)
    context={"tracks":page_obj}
    return render(request,"library/audio-book/audios.html",context)
...
def single_audio(request,slug):
    ...
    return render(request,"library/audio-book/single_track.html",context)
