...
from library.models import Library_Payment,Movies,Audio_Book,E_Book
...
from library.models import Audio_Book_Tracks,Audio_Tracks,Movies,E_Book
...
def edit_consultant_payment(request,id):
    payment=get_object_or_404(Cosultant_Payment,id=id,user=request.user,status="declined")
    if request.user == payment.user:
        if payment.method == "Western Union" or payment.method == "bank":
            ...
            if request.method == "POST":
                if form.is_valid():
                    ...
                    if image:
                        ...
                        try: 
                            if data["HttpCode"] == 201:
                                instance.payment_image = f"https://{agartha_cdn}/audio-book-payment/{track.slug}/{payment.user.username}/{image}"
                                instance.save()
                        except:
                            pass                 
                    instance.save()
